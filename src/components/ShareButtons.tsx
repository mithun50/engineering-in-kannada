...
export const ShareButtons: React.FC<ShareButtonsProps> = ({ url, title, text }) => {
      ...
      <div className="flex flex-wrap items-center gap-3">
        {socialPlatforms.map((platform) => (
          <a
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-md bg-white/10 hover:bg-white/20 border border-white/20 transition-all duration-200 group ${platform.color} hover:text-opacity-80`}
          >
            {React.cloneElement(platform.icon, { className: `group-hover:scale-110 transition-transform` })}
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-md bg-white/10 hover:bg-white/20 border border-white/20 text-gray-300 hover:text-white transition-all duration-200 group"
        >
          {copied ? <CheckCircle size={18} className="text-green-500 transition-all" /> : <LinkIcon size={18} className="group-hover:scale-110 transition-transform" />}
